...
export default function AdminTicketsPage() {
    ...
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-slate-950 pt-20 px-4 md:px-8 pb-24 text-slate-800 dark:text-slate-100">
            <div className="max-w-7xl mx-auto space-y-6">
                
                {/* Header */}
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        <h1 className="text-2xl font-bold tracking-tight">Pusat Tiket Komplain</h1>
                        <p className="text-slate-500 dark:text-slate-400 text-sm">Kelola keluhan, pantau chat teknisi-pelanggan, dan lakukan intervensi supervisor.</p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm flex items-center gap-4">
                        ...
                        <div>
                            <div className="text-xs text-slate-500 dark:text-slate-400 font-medium">Tiket Baru</div>
                    ...
                    <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm flex items-center gap-4">
                        ...
                        <div>
                            <div className="text-xs text-slate-500 dark:text-slate-400 font-medium">Sedang Diproses</div>
                    ...
                    <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm flex items-center gap-4">
                        ...
                        <div>
                            <div className="text-xs text-slate-500 dark:text-slate-400 font-medium">Terselesaikan</div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                    ...
                    {/* Ticket List */}
                    <div className="lg:col-span-5 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col h-[650px]">
                        ...
                        <div className="p-4 border-b border-slate-200 dark:border-slate-800 space-y-3">
                            <div className="relative">
                                ...
                                <input
                                    type="text"
                                    placeholder="Cari ID tiket, judul, atau pelanggan..."
                        ...
                        <div className="flex-1 overflow-y-auto divide-y divide-slate-100 dark:divide-slate-800">
                            {loading ? (
                                <div className="p-8 text-center text-slate-400 text-sm">Memuat tiket...</div>
                            ) : filteredTickets.length === 0 ? (
                                <div className="p-8 text-center text-slate-400 text-sm">Tidak ada tiket ditemukan.</div>
                            ) : (
                        ...
                        {selectedTicket ? (
                            <>
                                ...
                                <div className="p-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 flex flex-col md:flex-row md:items-center justify-between gap-4">
                                    <div>
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="text-xs text-slate-400">• Created: {new Date(selectedTicket.createdAt).toLocaleDateString()}</span>
                                        </div>
                                        <h2 className="font-bold text-base text-slate-800 dark:text-slate-100">{selectedTicket.title}</h2>
                                        <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Pelanggan: <span className="font-semibold">{selectedTicket.customer.name}</span> ({selectedTicket.customer.phone})</p>
                                    </div>
                                        ...
                                        >
                                            ...
                                            {staffList.map(staff => (
                                                <option key={staff.id} value={staff.id}>
                                                    {staff.fullName || staff.username} ({staff.role === 'admin' ? 'Admin' : staff.isTechnician ? 'Teknisi' : 'Agen'})
                                                </option>
                                            ))}
                                        </select>
                                    </div>
                                ...
                                <div className="px-4 py-3 bg-amber-50/50 dark:bg-amber-950/10 border-b border-amber-100/50 dark:border-amber-950/20 text-xs text-amber-800 dark:text-amber-300 flex gap-2">
                                    ...
                                    <div>
                                        <span className="font-semibold">Deskripsi Awal:</span> {selectedTicket.description}
                                    </div>
                                </div>
                                ...
                                <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50/30 dark:bg-slate-950/10">
                                    {chatLoading ? (
                                        <div className="text-center text-xs text-slate-400 pt-8">Memuat obrolan...</div>
                                    ) : messages.length === 0 ? (
                                        <div className="text-center text-xs text-slate-400 pt-8">Belum ada obrolan. Mulai obrolan di bawah.</div>
                                    ) : (
                                        messages.map(msg => {
                                ...
                                <form onSubmit={handleSendMessage} className="p-4 border-t border-slate-200 dark:border-slate-800 flex gap-2">
                                    <input
                                        type="text"
                                        placeholder="Kirim pesan intervensi sebagai Supervisor..."
                            ...
                            <div className="flex-1 flex flex-col items-center justify-center text-slate-400 p-8 space-y-3">
                                ...
                                <div className="text-sm">Pilih tiket komplain di daftar untuk membuka obrolan</div>
                            </div>
                        )}
                    </div>
