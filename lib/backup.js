...
export async function createBackup() {
    ...
    try {
        ...
        return new Promise((resolve, reject) => {
            ...
            // --- NEW: Full System Backup Files ---
            // JSON Configurations
            const appSettings = path.join(process.cwd(), 'app-settings.json');
            if (existsSync(appSettings)) archive.file(appSettings, { name: 'app-settings.json' });

            const billingSettings = path.join(process.cwd(), 'billing-settings.json');
            if (existsSync(billingSettings)) archive.file(billingSettings, { name: 'billing-settings.json' });
...
export async function restoreBackup(filename) {
    ...
    try {
        ...
        // 4. Restore Full System Backup Files
        console.log('Restoring extra configuration files...');
        
        // JSON Configs
        const appSetSrc = path.join(tempExtractDir, 'app-settings.json');
        if (existsSync(appSetSrc)) await fs.cp(appSetSrc, path.join(process.cwd(), 'app-settings.json'));

        const billSetSrc = path.join(tempExtractDir, 'billing-settings.json');
        if (existsSync(billSetSrc)) await fs.cp(billSetSrc, path.join(process.cwd(), 'billing-settings.json'));
