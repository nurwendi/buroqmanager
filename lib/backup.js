...
function parseDatabaseUrl() {
    ...
    // Regex to handle postgres://user:password@host:port/dbname?schema=public
...
async function dumpPostgres(outputPath) {
    ...
    return new Promise((resolve, reject) => {
        const dumpProcess = spawn(pgDumpPath, [
            ...
            '-p', config.port,
            '-U', config.user,
            '-F', 'p', // Plain SQL format (easier to debug/restore with psql)
            '--clean', // Include DROP commands
            '--if-exists',
            '--no-owner', // Skip ownership commands to avoid permission issues
...
async function restorePostgres(sqlFilePath) {
    ...
    return new Promise((resolve, reject) => {
        // We use psql to execute the SQL dump
        const restoreProcess = spawn(psqlPath, [
            '-h', config.host,
            '-p', config.port,
            '-U', config.user,
            '-d', config.database,
...
export async function restoreBackup(filename) {
    ...
    try {
        ...
        if (existsSync(publicDirInZip)) {
            ...
            // fs.cp (recursive) is available in Node 16.7+
            const { cp } = require('fs/promises');
            await cp(publicDirInZip, PUBLIC_DIR, { recursive: true, force: true });
            console.log('Public assets restored.');
        } else {
            // Handle legacy format or specific file checks if 'public' folder didn't exist in zip root
            // but files were added individually at root?
            // Our createBackup puts them in 'public/' prefix in zip, so extractAllTo preserves structure
            // So checking tempExtractDir/public is correct.
