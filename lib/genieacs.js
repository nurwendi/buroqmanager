...
export async function findDevices(query = {}) {
    // Default projection to get essential info
    const projection = {
        _id: 1,
        'DeviceID.SerialNumber': 1,
        'DeviceID.ProductClass': 1,
        'DeviceID.Manufacturer': 1,
        'InternetGatewayDevice.WANDevice.1.WANConnectionDevice.1.WANIPConnection.1.ExternalIPAddress': 1, // Common TR-069 path
    ...
    return await acsRequest(`/devices/?query=${queryString}&projection=${projectionString}`);
}
...
export async function rebootDevice(deviceId) {
    ...
    return await acsRequest(`/tasks/${deviceId}`, 'POST', task);
}
